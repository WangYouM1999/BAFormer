...
class DWLK(nn.Module):
    def __init__(self, dim):
        ...
        # self.conv2 = nn.Conv2d(dim, dim, 5, padding=4, groups=dim, dilation=2)
        ...
        # self.conv1 = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
        # self.conv2 = nn.Conv2d(dim, dim, 5, padding=4, groups=dim, dilation=2)
        # self.conv3 = nn.Conv2d(dim, dim, 7, padding=9, groups=dim, dilation=3)

        # (35,1) -> （5，1） + （11，3）
        # self.conv1 = nn.Conv2d(dim, dim, 5, padding=2, groups=dim)
        # self.conv2 = nn.Conv2d(dim, dim, 11, padding=15, groups=dim, dilation=3)
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        # self.dwconv = LSKAttention(hidden_features)
        self.dwconv = DWLK(hidden_features)
        self.act = act_layer()
...
class AdaptiveFeatureMixer(nn.Module):
    def __init__(self,
        ...
        # self.qkv_h = ConvBN(3 * dim, 3 * dim, kernel_size=3)
        self.qkv = Conv(dim, 3 * dim, kernel_size=1, bias=qkv_bias)
        self.local1 = ConvBN(dim, dim, kernel_size=3)
        self.local2 = ConvBN(dim, dim, kernel_size=1)

        # self.alpha = nn.Parameter(torch.Tensor([0.5]))  # 自适应融合的权重参数
        # self.proj = ConvBN(dim, dim, kernel_size=3)
        ...
        # self.ssf = AdaptSpatialFusion(dim)
...
class AdaptSpatialFusion(nn.Module):
    def __init__(self, dim):
        super().__init__()
    ...
    def forward(self, local_unit, global_unit):
        feat = local_unit + global_unit
        # 1.attention cat
        avg_attn = torch.mean(feat, dim=1, keepdim=True)
        max_attn, _ = torch.max(feat, dim=1, keepdim=True)
        attn = torch.cat([avg_attn, max_attn], dim=1)
        ...
        # 3.attention select
        out = local_unit * sig[:, 0, :, :].unsqueeze(1) + global_unit * sig[:, 1, :, :].unsqueeze(1)
        out = self.conv(out)
        return out
...
class Block(nn.Module):
    ...
    def freq_decompose(self, x):
        x_d = torch.mean(x, (-2), keepdim=True)  # [b, 1,c]
        x_h = x - x_d  # high freq [b, H*w, c]
        return x_d, x_h

    def forward(self, x):
        # B, C, H, W = x.size()
        # x_attn = self.attn(self.norm1(x))
        # x_attn = x_attn.view(B, C, H * W).transpose(1, 2)
...
if __name__ == "__main__":
    ...
    net = UNetFormer(num_classes=6, decode_channels=256, num_heads=16)
    # print(net)
    out = net(x)
