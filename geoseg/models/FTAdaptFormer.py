...
class AdaptiveFeatureMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        q_h = rearrange(q_h, 'b ( h d) (hh ws1) (ww ws2) -> (b hh ww) h (ws1 ws2) d',
                        h=self.num_heads, d=C // self.num_heads, hh=Hp // self.ws, ww=Wp // self.ws, ws1=self.ws,
...
class Block(nn.Module):
    def __init__(self, dim=256, num_heads=8, mlp_ratio=4., qkv_bias=False, drop=0.,
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp_DWLK(in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim, act_layer=act_layer,
                       drop=drop)
        self.norm2 = norm_layer(dim)
...
class FTAdaptFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            x, edge = self.decoder(res1, res2, res3, res4, h, w, self.num_classes)
            return x, edge
        else:
...
def ft_adaptformer(pretrained=True, num_classes=6, freeze_stages=-1, decoder_channels=256,
                  weight_path='pretrain_weights/stseg_base.pth'):
    model = FTAdaptFormer(num_classes=num_classes,
...
if __name__ == "__main__":
    ...
    net = ft_adaptformer(num_classes=2)
    # print(net)
    out = net(x)
