...
# 创建权重参数列表
WEIGHTS=("adaptformer-init-r18-768crop-ms-e45")  # 将起始权重参数添加到列表中
...
# 创建保存测试精度信息的文件
result_file="model_weights/potsdam/adaptformer-init-r18-768crop-ms-e45/${file_prefix}_test_results.txt"
> $result_file
...
do
    ...
    # 更新权重参数
    sed -i "s/test_weights_name = \".*\"/test_weights_name = \"$weight\"/g" ./config/potsdam/adaptformer.py

    # 执行测试命令，并将精度信息追加写入文件
    echo "执行测试，权重参数为: $weight.ckpt"
    echo "------------------------------------------------------"
    result=$(python ./potsdam_test.py -c ./config/potsdam/adaptformer.py -o fig_results/potsdam/diff/adaptformer --rgb -t 'lr')
